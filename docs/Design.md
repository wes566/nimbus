...
Veil aims to address these issues and provide a consistent
...
Veil's primary purpose is making native code callable from
...
directly. The veil bridge will provide a means to register the
...
Veil is primarily written in Swift and should support Swift as a
first-class language for writing bridged code. However due to lack of
a powerful reflection system in current versions of Swift (Mirror is
very limited) Veil relies on the Objective-C runtime to
introspect objects for methods that can be bridged.
